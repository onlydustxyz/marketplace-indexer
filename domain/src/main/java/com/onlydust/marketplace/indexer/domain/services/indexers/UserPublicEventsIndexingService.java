...
public class UserPublicEventsIndexingService implements UserPublicEventsIndexer {
    ...
    private void index(RawPublicEvent event) {
        index(event.actor());
        index(event.org());
        index(event.repo());
    ...
    private void index(RawPublicEvent.Payload rawPayload) {
    ...
    private void index(RawRepo repo) {
        rawStorageWriter.saveRepo(repo);
        repoIndexer.indexRepo(repo.getId());
    }

    private void index(RawPullRequest pullRequest) {
        final var repo = pullRequest.getHead().getRepo();
        rawStorageWriter.savePullRequest(repo.getId(), pullRequest);
        pullRequestIndexer.indexPullRequest(repo.getOwner().getLogin(), repo.getName(), pullRequest.getNumber());
    }

    private void index(RawPullRequestEventPayload event) {
