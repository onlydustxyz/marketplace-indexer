...
public class GithubRawStorageReader implements RawStorageReader, PublicEventRawStorageReader {
    ...
    public Optional<List<RawCodeReview>> pullRequestReviews(Long repoId, Long pullRequestId, Long prNumber) {
        final var page = new GithubPage<>(client, "/repositories/" + repoId + "/pulls/" + prNumber + "/reviews?per_page=100", RawCodeReview[].class);
    ...
    public Optional<RawCommit> commit(Long repoId, String sha) {
        final var commit = client.get("/repositories/" + repoId + "/commits/" + sha, RawCommit.class);

        if (commit.isEmpty())
            LOGGER.warn("Unable to fetch commit https://api.github.com/repositories/{}/commits/{}", repoId, sha);
