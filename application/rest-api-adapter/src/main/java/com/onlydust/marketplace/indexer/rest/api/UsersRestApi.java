...
public class UsersRestApi implements UsersApi {
    ...
    private final UserIndexer liveUserIndexer;
    ...
    @Override
    public ResponseEntity<Void> indexUser(Long userId, String authorization, Boolean forceRefresh) {
        userIndexingJobStorage.add(userId);

        authorizationContext.withAuthorization(authorization,
                () -> {
                    if (TRUE.equals(forceRefresh)) {
                        liveUserIndexer.indexUser(userId);
                    } else {
